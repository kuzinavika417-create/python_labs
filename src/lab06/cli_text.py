...
def main():
    parser = argparse.ArgumentParser(
        description="CLI-утилиты для работы с текстом (cat/stats)"
    )
    ...
    cat_parser.add_argument(
        "-n", action="store_true", help="Нумеровать строки (для команды cat)"
    )
    ...
    stats_parser.add_argument(
        ...
        type=int,
        default=None,
        help="Количество топ-слов для вывода (если указан, выполняется stats)",
    )
